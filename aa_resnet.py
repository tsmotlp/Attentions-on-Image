...
def aa_resnet50(pretrained=False, num_classes=10, **kwargs):
    """build a aa_resnet50 model
    args:
        pretrained(bool):if true, returns a model pretrained on ImageNet
    """
    layers = [3, 4, 6, 3]
    model = AA_ResNet(layers, num_classes, **kwargs)
    if pretrained:
        checkpoint = torch.load(model.model_path)
        model.load_state_dict(checkpoint['model'].state_dict())
    return model
def aa_resnet101(pretrained=False, num_classes=10, **kwargs):
    ...
    if pretrained:
        ...
        model.load_state_dict(checkpoint['model'].state_dict())
    return model
def aa_resnet152(pretrained=False, num_classes=10, **kwargs):
